<template>
  <div class="scroll-500">
    <div class="d-flex justify-content-end mb-3">
      <button v-if="hideCommentBox && showTopicModal" type="button" class="btn btn-primary me-1" data-bs-toggle="modal" data-bs-target="#createNewMessageModal">
        Create New Topic
      </button>
    </div>

    <!-- Message Input Box -->
    <div class="card mb-2 border" v-if="!hideCommentBox && !showTopicModal">
      <div class="card-body">
  ...
  <div class="modal fade" id="createNewMessageModal" tabindex="-1" aria-labelledby="createNewMessageModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="createNewMessageModalLabel">Create New Topic</h5>
          <button id="closeCreateNewMessageModal" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">

          <!-- Topic Title field -->
          <div class="form-group mt-2 mb-2">
            <input type="text" v-model="messageTitle" class="form-control" placeholder="Enter topic title here" />
          </div>
          ...
          <div class="d-flex justify-content-between">
            ...
            <div>
              ...
              <button
                v-if="isActivated && isSupportedChain && hasDomainOrNotRequired"
                :disabled="!messageText || !messageTitle || waitingCreateMessage"
                class="btn btn-primary me-2 mt-2"
                @click="createMessage"
              >
                <span v-if="waitingCreateMessage" class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
...
export default {
  ...
  props: [
    ...
    'showTopicModal', // if true, we'll show the topic modal
  ],
  ...
  methods: {
    ...
    async createMessage() {
      ...
      const storageUrl = await this.uploadToChatStorage()
      ...
      try {
        ...
        if (receipt.status === 1) {
          ...
          document.getElementById('closeCreateNewMessageModal').click()
        } else {
          this.toast.dismiss(toastWait)
